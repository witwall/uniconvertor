...
from uc2.formats.riff.utils import dword2py_int, double2py_float, \
							long2py_float, word2py_int
from uc2.formats.cdr.const import CDR6, CDR7, CDR8, CDR9, CDR10, \
								CDR11, CDR12, CDR13, cdrunit_to_pt, \
								CDR_COLOR_CMYK, CDR_COLOR_BGR, CDR_COLOR_CMY, \
								CDR_COLOR_CMYK255, CDR_COLOR_GRAY, CDR_COLOR_LAB, \
								CDR_COLOR_REGISTRATION, CDR_COLOR_CMYK2
...
from uc2.formats.cdr.utils import parse_matrix, parse_cmy, parse_cmyk, \
							parse_cmyk255, parse_lab, parse_reg_color, \
							parse_bgr, parse_grayscale, parse_size_value
...
class CdrFillProperty(RiffObject):
	...
	def update(self):
		...
		cst = self.color_space_type
		if cst == CDR_COLOR_CMYK or cst == CDR_COLOR_CMYK2:
			self.fill_color = parse_cmyk(self.chunk[offset:offset + 4])
			self.cache_fields.append((offset, 4, 'color value'))
		elif cst == CDR_COLOR_CMYK255 or cst == 0x11 or cst == 1:
			self.fill_color = parse_cmyk255(self.chunk[offset:offset + 4])
			self.cache_fields.append((offset, 4, 'color value'))
		elif cst == CDR_COLOR_CMY:
			self.fill_color = parse_cmy(self.chunk[offset:offset + 3])
			self.cache_fields.append((offset, 3, 'color value'))
		elif cst == CDR_COLOR_BGR:
			self.fill_color = parse_bgr(self.chunk[offset:offset + 3])
			self.cache_fields.append((offset, 3, 'color value'))
		elif cst == CDR_COLOR_GRAY:
			self.fill_color = parse_grayscale(self.chunk[offset])
			self.cache_fields.append((offset, 1, 'color value'))
		elif cst == CDR_COLOR_LAB:
			self.fill_color = parse_lab(self.chunk[offset:offset + 3])
			self.cache_fields.append((offset, 3, 'color value'))
		elif cst == CDR_COLOR_REGISTRATION:
			self.fill_color = parse_reg_color(self.chunk[offset:offset + 4])
		else:
			self.fill_color = []
...
class CdrOutlineProperty(RiffObject):
	...
	def update(self):
		...
		self.color_space_type = ord(data[offset + 48])
		self.cache_fields.append((lc_offset + 56, 1, 'color space type'))

		offset += 0x38
		cst = self.color_space_type
		if cst == CDR_COLOR_CMYK or cst == CDR_COLOR_CMYK2:
			self.stroke_color = parse_cmyk(self.chunk[offset:offset + 4])
			self.cache_fields.append((offset + 8, 4, 'color value'))
		elif cst == CDR_COLOR_CMYK255 or cst == 0x11:
			self.stroke_color = parse_cmyk255(self.chunk[offset:offset + 4])
			self.cache_fields.append((offset + 8, 4, 'color value'))
		elif cst == CDR_COLOR_CMY:
			self.stroke_color = parse_cmy(self.chunk[offset:offset + 3])
			self.cache_fields.append((offset + 8, 3, 'color value'))
		elif cst == CDR_COLOR_BGR:
			self.stroke_color = parse_bgr(self.chunk[offset:offset + 3])
			self.cache_fields.append((offset + 8, 3, 'color value'))
		elif cst == CDR_COLOR_GRAY:
			self.stroke_color = parse_grayscale(self.chunk[offset])
			self.cache_fields.append((offset + 8, 1, 'color value'))
		elif cst == CDR_COLOR_LAB:
			self.stroke_color = parse_lab(self.chunk[offset:offset + 3])
			self.cache_fields.append((offset + 8, 3, 'color value'))
		elif cst == CDR_COLOR_REGISTRATION:
			self.stroke_color = parse_reg_color(self.chunk[offset:offset + 4])
		else:
			self.stroke_color = []
