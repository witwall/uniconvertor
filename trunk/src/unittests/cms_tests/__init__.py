...
class TestCmsFunctions(unittest.TestCase):
	...
	def setUp(self):
		...
		self.transform = libcms.cms_create_transform(self.inProfile,
						const.TYPE_RGBA_8, self.outProfile, const.TYPE_CMYK_8,
						const.INTENT_PERCEPTUAL, const.cmsFLAGS_NOTPRECALC)
	...
	def test04_create_transform(self):
		self.assertNotEqual(None, libcms.cms_create_transform(self.inProfile,
				const.TYPE_RGB_8, self.outProfile, const.TYPE_CMYK_8))
		self.assertNotEqual(None, libcms.cms_create_transform(self.inProfile,
				const.TYPE_RGBA_8, self.outProfile, const.TYPE_CMYK_8))
		self.assertNotEqual(None, libcms.cms_create_transform(self.outProfile,
				const.TYPE_CMYK_8, self.inProfile, const.TYPE_RGBA_8))
		self.assertNotEqual(None, libcms.cms_create_transform(self.outProfile,
				const.TYPE_CMYK_8, self.inProfile, const.TYPE_RGB_8))

	def test05_create_transform_with_custom_intent(self):
		self.assertNotEqual(None, libcms.cms_create_transform(self.inProfile,
				const.TYPE_RGB_8, self.outProfile, const.TYPE_CMYK_8,
				const.INTENT_PERCEPTUAL))
		self.assertNotEqual(None, libcms.cms_create_transform(self.inProfile,
				const.TYPE_RGB_8, self.outProfile, const.TYPE_CMYK_8,
				const.INTENT_RELATIVE_COLORIMETRIC))
		self.assertNotEqual(None, libcms.cms_create_transform(self.inProfile,
				const.TYPE_RGB_8, self.outProfile, const.TYPE_CMYK_8,
				const.INTENT_SATURATION))
		self.assertNotEqual(None, libcms.cms_create_transform(self.inProfile,
				const.TYPE_RGB_8, self.outProfile, const.TYPE_CMYK_8,
				const.INTENT_ABSOLUTE_COLORIMETRIC))

	def test06_create_transform_with_custom_flags(self):
		self.assertNotEqual(None, libcms.cms_create_transform(self.inProfile,
				const.TYPE_RGB_8, self.outProfile, const.TYPE_CMYK_8,
				const.INTENT_PERCEPTUAL,
				const.cmsFLAGS_NOTPRECALC | const.cmsFLAGS_GAMUTCHECK))
		self.assertNotEqual(None, libcms.cms_create_transform(self.inProfile,
				const.TYPE_RGB_8, self.outProfile, const.TYPE_CMYK_8,
				const.INTENT_PERCEPTUAL,
				const.cmsFLAGS_PRESERVEBLACK | const.cmsFLAGS_BLACKPOINTCOMPENSATION))
		self.assertNotEqual(None, libcms.cms_create_transform(self.inProfile,
				const.TYPE_RGB_8, self.outProfile, const.TYPE_CMYK_8,
				const.INTENT_PERCEPTUAL,
				const.cmsFLAGS_NOTPRECALC | const.cmsFLAGS_HIGHRESPRECALC))
		self.assertNotEqual(None, libcms.cms_create_transform(self.inProfile,
				const.TYPE_RGB_8, self.outProfile, const.TYPE_CMYK_8,
				const.INTENT_PERCEPTUAL,
				const.cmsFLAGS_NOTPRECALC | const.cmsFLAGS_LOWRESPRECALC))

	def test07_create_transform_with_invalid_intent(self):
		self.assertNotEqual(None, libcms.cms_create_transform(self.inProfile,
				const.TYPE_RGB_8, self.outProfile, const.TYPE_CMYK_8, 3))
		try:
			libcms.cms_create_transform(self.inProfile, const.TYPE_RGB_8,
									self.outProfile, const.TYPE_CMYK_8, 4)
		except libcms.CmsError:
			return
		self.fail()
	...
	def test08_create_proofing_transform(self):
		self.assertNotEqual(None, libcms.cms_create_proofing_transform(self.inProfile,
				const.TYPE_RGB_8, self.inProfile, const.TYPE_RGB_8, self.outProfile))
		self.assertNotEqual(None, libcms.cms_create_proofing_transform(self.inProfile,
				const.TYPE_RGBA_8, self.inProfile, const.TYPE_RGB_8, self.outProfile))
		self.assertNotEqual(None, libcms.cms_create_proofing_transform(self.inProfile,
				const.TYPE_RGB_8, self.inProfile, const.TYPE_RGBA_8, self.outProfile))
		self.assertNotEqual(None, libcms.cms_create_proofing_transform(self.inProfile,
				const.TYPE_RGBA_8, self.inProfile, const.TYPE_RGBA_8, self.outProfile))
	...
	def test10_create_proofing_transform_with_custom_proofing_intent(self):
		self.assertNotEqual(None, libcms.cms_create_proofing_transform(self.inProfile,
				const.TYPE_RGB_8, self.inProfile, const.TYPE_RGB_8, self.outProfile,
				const.INTENT_PERCEPTUAL, const.INTENT_PERCEPTUAL))
		self.assertNotEqual(None, libcms.cms_create_proofing_transform(self.inProfile,
				const.TYPE_RGB_8, self.inProfile, const.TYPE_RGB_8, self.outProfile,
				const.INTENT_PERCEPTUAL, const.INTENT_RELATIVE_COLORIMETRIC))
		self.assertNotEqual(None, libcms.cms_create_proofing_transform(self.inProfile,
				const.TYPE_RGB_8, self.inProfile, const.TYPE_RGB_8, self.outProfile,
				const.INTENT_PERCEPTUAL, const.INTENT_SATURATION))
		self.assertNotEqual(None, libcms.cms_create_proofing_transform(self.inProfile,
				const.TYPE_RGB_8, self.inProfile, const.TYPE_RGB_8, self.outProfile,
				const.INTENT_PERCEPTUAL, const.INTENT_ABSOLUTE_COLORIMETRIC))
	...
	def test12_create_proofing_transform_with_invalid_intent(self):
		self.assertNotEqual(None, libcms.cms_create_proofing_transform(self.inProfile,
				const.TYPE_RGB_8, self.inProfile, const.TYPE_RGB_8, self.outProfile, 3))
		try:
			libcms.cms_create_proofing_transform(self.inProfile, const.TYPE_RGB_8,
				self.inProfile, const.TYPE_RGB_8, self.outProfile, 4)
		except libcms.CmsError:
			return
		self.fail()
	...
	def test13_create_proofing_transform_with_invalid_proofing_intent(self):
		self.assertNotEqual(None, libcms.cms_create_proofing_transform(self.inProfile,
				const.TYPE_RGB_8, self.inProfile, const.TYPE_RGB_8, self.outProfile, 1, 2))
		try:
			libcms.cms_create_proofing_transform(self.inProfile,
				const.TYPE_RGB_8, self.inProfile, const.TYPE_RGB_8, self.outProfile, 1, 4)
		except libcms.CmsError:
			return
		self.fail()
